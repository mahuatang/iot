...
import com.soco.car.iot.server.app.message.response.BaseResponseMsg;
...
public class AuthIntercepter {
	...
	@Around(value = "execution(* com.soco.car.iot.server.car.message.*.convert.*.*(..)) && @annotation(auth) ||"
			+ "execution(* com.soco.car.iot.service.app.*.*(..)) && @annotation(auth)")
