...
import com.soco.car.iot.server.car.message.msg.AutoTimeMsg;
...
public class AutoTimeConvert implements MessageConverter {
	...
	public BaseMsg convert2Message(Channel channel, byte[] msg) {
		...
		byte[] content = ByteUtils.addAll(new byte[] { 0x0b }, new byte[] { supportProcotol() }, getNowTimeBytes(),
				messageSerialNumber);
		...
		autoTimeMsg.setRequestContent(msg);
		return autoTimeMsg;
	}
